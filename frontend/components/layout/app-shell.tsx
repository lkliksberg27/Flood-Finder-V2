...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden bg-[#0c1021]">
      ...
      {/* Bottom tab bar */}
      <nav
        className="fixed bottom-0 inset-x-0 z-[2000] bg-[#0c1021]/92 backdrop-blur-2xl border-t border-white/[0.05]"
          ...
          {TABS.map(({ href, label, Icon }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                className={cn(
                  "flex flex-col items-center gap-[1px] py-1.5 px-5 rounded-2xl select-none transition-all active:scale-[0.82]",
